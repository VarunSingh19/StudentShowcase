...
import { updateDoc, doc, Timestamp, query, collection, getDocs, getDoc } from 'firebase/firestore';
import { db } from '@/lib/firebase';
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { useToast } from '@/hooks/use-toast';
import { ShoppingCart, Clock, Truck, CheckCircle, Package, User, MapPin, Box, Calendar, DollarSign, Phone, Mail } from 'lucide-react';
...
export function OrderList({ orders: initialOrders, onOrderUpdated }: OrderListProps) {
        ...
        try {
            ...
            const ordersWithDetails = await Promise.all(
                initialOrders.map(async (order) => {
                    try {
                        const userProfileDoc = await getDoc(doc(db, 'profiles', order.userId));
    ...
    const formatDate = (timestamp: Timestamp | Date) => {
        return new Date(timestamp instanceof Timestamp ? timestamp.toDate() : timestamp).toLocaleDateString();
    };
                                        ...
                                        <Card className="overflow-hidden hover:shadow-lg transition-shadow">
                                            <AccordionTrigger className="px-6 py-4 hover:no-underline">
                                                <div className="flex justify-between items-center w-full">
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <HeaderImageModal
                                                            src={order.userProfile?.avatarUrl || "/placeholder.svg"}
                                                            ...
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    <h4 className="font-semibold text-lg">
                                                                        <Link href={`/profile/${order.userId}`} passHref>
                                                                            {order.userProfile?.displayName || 'Customer'}
                                                                        </Link>
                                                                    </h4>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="space-y-4">
                                                                {order.products.map((item) => {
                                                                    ...
                                                                    return product ? (
                                                                        <div key={item.productId} className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg p-4">
