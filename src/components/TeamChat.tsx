...
export function TeamChat({
    ...
    useEffect(() => {
        ...
        channel.bind('new-message', async (data: Message) => {
            ...
            if (data.userId !== currentUserId) {
                await updateUnreadCount(teamId, currentUserId);
            }
        });
        channel.bind('message-updated', (data: Message) => {
            setMessages((prevMessages) => {
        ...
        const fetchMessages = async () => {
            try {
                ...
                await updateUnreadCount(teamId, currentUserId, true);
            } catch (error) {
    ...
    const updateUnreadCount = async (teamId: string, userId: string, reset: boolean = false) => {
        const teamRef = doc(db, 'teams', teamId);
        const teamDoc = await getDoc(teamRef);
        const currentMembers = teamDoc.data()?.currentMembers || [];

        const updates: { [key: string]: any } = {};
        currentMembers.forEach((memberId: string) => {
            if (memberId !== userId) {
                if (reset) {
                    updates[`unreadMessages.${memberId}`] = 0;
                } else {
                    updates[`unreadMessages.${memberId}`] = increment(1);
                }
            }
        });
                            ...
                            >
                                ...
                                <div className={`flex flex-col space-y-1 ${isCurrentUser ? 'items-end' : ''}`}>
                                    ...
                                    <div className="flex items-center gap-1 mt-1">
                                        ...
                                        {message.readBy.length === Object.keys(userProfiles).length ? (
                                            <Check className="h-4 w-4 text-blue-500" />
                                        ) : (
                                            <Check className="h-4 w-4 text-gray-500" />
                                        )}
